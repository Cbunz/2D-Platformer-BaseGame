        ...
        do
        {
            for (int i = 0, count = caughtObjects.Count; i < count; i++)
            {
                ...
                if (!caughtObject.inContact)
                {
                    ...
                    bool verticalAlignment = (caughtObjectCollider.bounds.max.x > _collider.bounds.min.x) && (caughtObjectCollider.bounds.min.x < _collider.bounds.max.x);
                    if (verticalAlignment)
                    {
                        float yDiff = caughtObjects[i].collider.bounds.min.y - _collider.bounds.max.y;
